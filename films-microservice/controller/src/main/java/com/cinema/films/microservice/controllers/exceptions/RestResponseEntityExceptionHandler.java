package com.cinema.films.microservice.controllers.exceptions;
...
public class RestResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    protected ResponseEntity<Object> minIoException(Exception e, WebRequest request) {
        Map<String, Object> responseBody = new LinkedHashMap<>();
        log.error("MinIo exception! " + e.getMessage());

        responseBody.put("message", String.format("MinIo exception! %s", e.getMessage()));
        responseBody.put("time", LocalDateTime.now());
        return new ResponseEntity<>(responseBody, HttpStatus.INTERNAL_SERVER_ERROR);
    }
